...
public class XxlCfgClient implements Watcher {
	...
	public XxlCfgClient(){
		try {
			this.zooKeeper = new ZooKeeper(Environment.ZK_ADDRESS, 2000, this);
			this.create(Environment.CONF_DATA_PATH);	// init cfg root path
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private String generateZnodeKeyFromPath(String znodePath){
		if (znodePath.length() <= Environment.CONF_DATA_PATH.length()) {
			return null;
		}
		return znodePath.substring(Environment.CONF_DATA_PATH.length()+1, znodePath.length());
	}
	...
	private String generateZnodePathFromKey(String ZnodeKey){
		return Environment.CONF_DATA_PATH + "/" + ZnodeKey;
	}
	...
	public Map<String, String> getAllData(){
		...
		try {
			List<String> nodeNameList = this.zooKeeper.getChildren(Environment.CONF_DATA_PATH, true);
			if (CollectionUtils.isNotEmpty(nodeNameList)) {
				for (String znodeKey : nodeNameList) {
					String znodeValue = this.getData(znodeKey);
					allData.put(znodeKey, znodeValue);
				}
			}
		} catch (KeeperException e) {
			e.printStackTrace();
	...
	public static void main(String[] args) throws InterruptedException, KeeperException {
		...
		Stat stat = client.zooKeeper.exists(Environment.CONF_DATA_PATH, true);
		client.zooKeeper.delete(Environment.CONF_DATA_PATH, stat.getVersion());
	}
