...
- 环境配置文件，支持自定义存放位置，项目resource下或磁盘目录下均可；
- 支持设置ZK中配置存储路径，方便实现多环境复用ZK集群；
- 升级依赖版本，如Ehcache、Spring等；
- 重构核心代码，规范代码结构； 
- 主动缓存null或不存在类型配置，避免配置请求穿透到ZK引发雪崩问题；
- Local Cache缓存长度固定为1000，采用LRU策略移除。
- 新增守护线程，周期性刷新Local Cache中配置数据并watch，进一步提高配置时效性；
- ZK过期重连时，主动刷新LocalCache中配置数据，提高异常情况下配置时效性；
- 支持通过 "@XxlConf" 注解获取配置，并且支持动态推送更新；
