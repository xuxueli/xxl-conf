...
- 5、【优化】配置编辑器升级为CodeMirror，提升交互体验；
- 6、【ING】配置历史Diff查看；
- 7、【ING】配置onChange调整为异步线程处理，避免耗时监听逻辑影响性能；
- 8、【ING】首页报表数据处理；
- 9、【ING】集群管理：支持自定义并管理集群（Cluster），系统配置下钻到集群维度，完整维度为 “env-clustor-appname”;
- 10、【ING】配置中心以及客户端重构：
  - 服务端：配置“预热/刷新/增量更新”是，底层数据结构标准化，格式为：env-clustor-appkey -> 配置TreeMap；
  - 客户端：
    - 预热：启动时全量Page查询，批量同步内存；
    - 刷新：定期全量检测配置md5摘要，不一致数据主动刷新；
    - 增量：“env-clustor-appkey” 维度监听，配置变更时触发全量md5摘要比对，触发更新；
    - 降低：配置全量数据，定期写本地file（“/xxl-conf/env/clustor/confdata/appkey.prop”）；启动预热时联系三次失败，降级使用本地配置。


### TODO LIST
- Cluster/集群管理：新增“集群”维度，针对配置、注册细化维度为 “env-clustor-appkey”；客户端与服务端按照最细维度配置、使用，集群间相互隔离；
- 配置数据存储、同步：
  - 服务端：key=env-clustor-appkey；值全量放treeMap。大内存。
  - 客户端：
    - 预热：分页page查询，查询服务端treemap；客户端全量缓存；
    - 降级：成功写file，预热失败3次降级 /xxl-conf/confdata/env/clustor/appkey.prop
    - 定时：全量校验md5，识别不一致key，异步更新。
    - 增量：单key维度，
