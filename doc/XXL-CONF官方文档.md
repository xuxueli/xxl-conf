...
- 16、高性能: 通过Ehcache对ZK推送的配置做Local Cache, 提高性能;
- 17、客户端断线重连强化：除了依赖ZK之外，设置守护线程，提高异常情况下配置稳定性和时效性；
- 18、空配置处理：主动缓存null或不存在类型配置，避免配置请求穿透到ZK引发雪崩问题；
...
- 4、配置Agent服务，支持 long-polling 方式加载配置，并进行权限校验；
